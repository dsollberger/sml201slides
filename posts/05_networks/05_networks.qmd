...
Which of the following is the *intersection* of sets $A$ and $B$?
...
this_prompt <- "Which of the following is the intersection of sets A and B?"
this_choices <- c("X", answer = "Y")
cat(this_prompt, 
    webexercises::longmcq(this_choices))
```
...
Consider the following code chains.
...
df |>
  group_by(var1) |>
  mutate(stat1 = mean(var1, na.rm = TRUE)) |>
  ungroup() |>
  select(var1, stat1) |>
  arrange(desc(stat1))
...
df |>
  group_by(var1) |>
  mutate(stat1 = sd(var1, na.rm = TRUE)) |>
  ungroup() |>
  select(var1, stat1) |>
  arrange(desc(stat1))
...
df |>
  group_by(var1) |>
  mutate(stat1 = median(var1, na.rm = TRUE)) |>
  ungroup() |>
  select(var1, stat1) |>
  arrange(desc(stat1))
...
df |>
  group_by(var1) |>
  mutate(stat1 = var(var1, na.rm = TRUE)) |>
  ungroup() |>
  select(var1, stat1) |>
  arrange(desc(stat1))
...
#| eval: true
#| results: asis
this_prompt <- "Which code chain computes the group means and displays them in descending order"
this_choices <- c(answer = "A", "B", "C", "D", "E")
cat(this_prompt, 
    webexercises::longmcq(this_choices))
```
...
#| eval: true
#| results: asis
this_prompt <- "Which code chain computes the mode of var1 and places at the top of the data frame?"
this_choices <- c("A", "B", "C", "D", answer = "E")
cat(this_prompt, 
    webexercises::longmcq(this_choices))
```
...
```{r}
set.seed(201)
example_network <- data.frame(
  from = sample(LETTERS[1:5],
                size = 14, replace = TRUE),
  to   = sample(LETTERS[1:5],
                size = 14, replace = TRUE)
)

node_tbl <- tibble(id = LETTERS[1:5],
                  label = LETTERS[1:5],
                  color = "blue",
                  shape = "circle")

edge_tbl <- tibble(
  from = example_network$from,
  to = example_network$to,
  color = "red") |>
  filter(from != to)

visNetwork(nodes = node_tbl, 
           edges = edge_tbl,
           main = "Example Network")
```
...
this_prompt <- "How many nodes are there in this network"
this_choices <- c(answer = "5", "9")
cat(this_prompt, 
    webexercises::longmcq(this_choices))
```
...
this_prompt <- "How many edges are there in this network?"
this_choices <- c("5", answer = "9")
cat(this_prompt, 
    webexercises::longmcq(this_choices))
```
...
<!---
[For optional practice] **Grade**: `r total_correct()`.
--->
