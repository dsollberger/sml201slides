...
![temporal data split](temporal_train_test.png)

* image source: [Saurabh Verma ](https://rpubs.com/vermasaurabh19/1156599)
...
For assurance in the results of ANOVA (analysis of variance), researchers usually check the following assumptions
...
4. **Random sampling**: Observations within each group have been sampled randomly and are independent of each other.

* source: [Editage](https://www.editage.com/blog/anova-types-uses-assumptions-a-quick-guide-for-biomedical-researchers/)

:::

So, how do we check if our data is normally distributed?
...
bike_df |>
  ...
  stat_qq_line() +
  labs(title = "QQ Plot",
       subtitle = "Height looks to be normally distributed\nexcept for lower level amounts",
...
bike_df |>
  ...
  stat_qq_line() +
  labs(title = "QQ Plot",
       subtitle = "Weight looks to be normally distributed",
       caption = "Source: Dream House Finance",
       x = "quantile 1", y = "quantile 2") +
  theme_minimal()
```
...
bike_df |>
  ...
  stat_qq_line() +
  labs(title = "QQ Plot",
       subtitle = "Age looks to be normally distributed",
       caption = "Source: Dream House Finance",
       x = "quantile 1", y = "quantile 2") +
  theme_minimal()
```
...
```{r}
stats::shapiro.test(bike_df$height)
```

We *reject* the null hypothesis that the height data was normally distributed.


```{r}
stats::shapiro.test(bike_df$weight)
```

We *fail to reject* the null hypothesis that the weight data was normally distributed.


```{r}
stats::shapiro.test(bike_df$age)
```

We *fail to reject* the null hypothesis that the age data was normally distributed.


# Pitch

![data science is growing!](growing_occupations.png)

* image source: [BLS Employment Growth](https://www.bls.gov/emp/)
