...
For every increase in year, the number of judicial referrals `r ifelse(b > 0, "increasing", "decreasing")` by about `r round(b,2)` referrals.
...
#| echo: false
#| eval: true
cat(paste0(round(a,3), " + (", round(b,3), ")x"))
```
...
#| eval: true
# properly programmatically
cat(predict(lin_fit, newdata = data.frame(year = 2024)))
```
